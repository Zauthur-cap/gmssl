 ...
 * @param count 大数的长度
 */
static void gm_i_bn_add_x(gm_bn_t rum, const gm_bn_t a, const gm_bn_t b, int count) {
   /* int i;
    rum[0] = a[0] + b[0];
     for(i = 1; i < count; i++) {
        rum[i] = a[i] + b[i] + (rum[i - 1] >> 32);
        rum[i - 1] &= 0x0FFFFFFFFULL;
     }
     */
    asm volatile (
        "mov x5, #0\n"
	...
	"ldr x4,[%[rum],x6,lsl #3]\n"
        "str x3, [%[rum], x5, lsl #3]\n"
        "and x4, x4, #0x0FFFFFFFFULL\n"
        "str x4, [%[rum], x6, lsl #3]\n"

        "add x5, x5, #1\n"  // Increment loop counter
        "cmp x5, %[count]\n"  // Compare loop counter with count
        "b.lt loop_start\n"  // Branch back to loop_start if less than
	

        : 
        : [rum] "r" (rum), [a] "r" (a), [b] "r" (b),[count] "r"(count)
...
/*static void gm_i_bn_sub(gm_bn_t rum, const gm_bn_t a, const gm_bn_t b) {
    asm volatile (
        "mov x5, #0\n"
        "ldr x0, [%[a], x5, lsl #3]\n"  // a[0]
        "ldr x1, [%[b], x5, lsl #3]\n"  // b[0]
	...
	"add x7, x7, x0\n"  // r[0] = r[0] + 2^32
        ...
        "sub x7, x0, x1\n"  // Compute a[i] - b[i]
        "add x7, x7, x2\n"  // Add r[i - 1] >> 32
        //"add x7, x7, #0x0FFFFFFFFULL\n"  // r[i] = r[i] + 2^32
	"str x7, [%[rum], x5, lsl #3]\n"
	"ldr x2, [%[rum], x6, lsl #3]\n"  // Load r[i - 1]
 	"and x2, x2, #0x0FFFFFFFFULL\n"
	"str x2, [%[rum], x6, lsl #3]\n"
        "add x5, x5, #1\n"  // Increment loop counter
        "cmp x5, %[count]\n"  // Compare loop counter with count
        "b.lt loop_start01\n"  // Branch back to loop_start if less than
	...
	"str x2, [%[rum], x6, lsl #3]\n"
        : 
        : [rum] "r" (rum), [a] "r" (a), [b] "r" (b), [count] "r" (GM_BN_ARR_SIZE_SUB_ONE)
        : "x0", "x1", "x2", "x5", "x6", "x7", "memory"
    );
}*/
/**
 * 大数乘法, r = k[b]
...
void gm_point_mul(gm_point_t *r, const gm_bn_t k, const gm_point_t *p) {
    gm_point_t q;
    gm_point_set_infinity(r);

    char bits[256] = {0};
    gm_bn_to_bits(k, bits);

    int window_size = 4;  // 窗口大小，根据需要进行调整

    // 创建窗口表，用于存储预先计算的点坐标
    gm_point_t window_table[16];
    gm_point_set_infinity(&window_table[0]);
    gm_point_copy(&window_table[1], p);

    for (int i = 2; i < 16; i++) {
        gm_point_add(&window_table[i], &window_table[i - 1],p);
    }

    for (int i = 0; i < 256; i += window_size) {
        gm_point_double(r, r);gm_point_double(r, r);gm_point_double(r, r);gm_point_double(r, r);
        int index = 0;
        for (int j = 0; j < window_size; j++) {
            index = (index << 1) | (bits[i + j] - '0');
        }
        gm_point_add(r, r, &window_table[index]);
    }
}
