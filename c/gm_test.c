...
void test_gm_sm3(const unsigned char* input, unsigned int iLen, const unsigned char* output_hex) {
    ...
    unsigned int message_length = 32 * 100000; // Total message length for 100,000 iterations
    ...
    for (i = 0; i < 100000; i++) { // 100,000 iterations
        gm_sm3(buf, 31, buf);
    ...
    throughput = (unsigned int)((message_length * 8) / (cpu_time_used * 1000000)); // Mbps calculation
