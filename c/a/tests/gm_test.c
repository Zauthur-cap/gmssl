...
void test_gm_sm3(const unsigned char *input, unsigned int iLen, const unsigned char *output_hex) {
    ...
    throughput = (unsigned int) ((message_length * 8) / (cpu_time_used * 1)); // Mbps calculation
...
//cbc模式
void test_gm_sm4_cbc(const unsigned char *key, int forEncryption, const unsigned char *input) {
    int i = 0;
    unsigned char buf[16] = { 0 };
    ...
    if (forEncryption) {
        ...
        gm_sm4_crypt(key, forEncryption, buf, buf);
        memcpy(iv, buf, 16); // Update the IV for the next block
    }
    else {
        // Decryption in CBC mode
        unsigned char temp[16]; // Temporary storage for the previous block's ciphertext
        memcpy(temp, buf, 16);
        gm_sm4_crypt(key, forEncryption, buf, buf);
        for (i = 0; i < 16; i++) {
            buf[i] ^= iv[i]; // XOR with the previous block's ciphertext or IV
        }
        memcpy(iv, temp, 16); // Update the IV for the next block
    }

    for (i = 0; i < 16; i++) {
        sprintf(res + i * 2, "%02X", (buf[i] & 0xFF));
    }
