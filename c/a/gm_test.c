...
void test_gm_sv_once( const char *key_hex, const  char *pubKey_hex, const char *sig_hex,
                  const    unsigned char *dgst_bytes, int algType) {
    unsigned char sig_res[256] = {0};
    gm_bn_t testK;
    gm_bn_t key;
    gm_bn_t dgst;
...
//sm3接口
void test_gm_sm3_once(const unsigned char *input, unsigned int iLen) {
    int i = 0;
    unsigned char buf[32] = {0};
    char res[65] = {0};
    gm_sm3_context ctx;
...
void test_gm_sm3(const unsigned char *input, unsigned int iLen, const unsigned char *output_hex) {
    ...
    pthread_t th8;
    start = clock();
    pthread_create(&th1,NULL,sm3_thread1,"DC7E07FF06247D00B4A8D1837C8F8B2A26C3C67C2EEE81B1E7CF9400B51891CB");
    pthread_create(&th2,NULL,sm3_thread2,"DC7E07FF06247D00B4A8D1837C8F8B2A26C3C67C2EEE81B1E7CF9400B51891CB");
    pthread_create(&th3,NULL,sm3_thread3,"DC7E07FF06247D00B4A8D1837C8F8B2A26C3C67C2EEE81B1E7CF9400B51891CB");
    pthread_create(&th4,NULL,sm3_thread4,"DC7E07FF06247D00B4A8D1837C8F8B2A26C3C67C2EEE81B1E7CF9400B51891CB");
    pthread_create(&th5,NULL,sm3_thread5,"DC7E07FF06247D00B4A8D1837C8F8B2A26C3C67C2EEE81B1E7CF9400B51891CB");
    pthread_create(&th6,NULL,sm3_thread6,"DC7E07FF06247D00B4A8D1837C8F8B2A26C3C67C2EEE81B1E7CF9400B51891CB");
    pthread_create(&th7,NULL,sm3_thread7,"DC7E07FF06247D00B4A8D1837C8F8B2A26C3C67C2EEE81B1E7CF9400B51891CB");
    pthread_create(&th8,NULL,sm3_thread8,"DC7E07FF06247D00B4A8D1837C8F8B2A26C3C67C2EEE81B1E7CF9400B51891CB");
    //gm_sm3_done(&ctx, buf);
    ...
    //printf("cputime = %f\n",cpu_time_used);
    unsigned int message_length = 32 * (80); // Total message length for 100,000 iterations
    throughput = (unsigned int) ((message_length * 8) / (cpu_time_used / 8)); // Mbps calculation
...
//cbc模式
// cbc模式
void test_gm_sm4_cbc(const unsigned char *key, int forEncryption,
                     const unsigned char *input) {
	...
	long filesize = ftell(file_input);
	fseek(file_input,0,SEEK_SET);

	unsigned char* f= (char*)malloc(filesize+1);
    fread(f, sizeof(char), filesize, file_input);
 //转化成2进制
    if (f != NULL && strlen(f) > 0) {
        size_t size = strlen(f) / 2;
        int i;
        char szchar[3];  

        int itemp;
        for (i = 0; i < size; i++) {
            strncpy(szchar, &f[i * 2], 2);
            szchar[2] = '\0';
            sscanf(szchar, "%x", &itemp);
            buf0[i] = (unsigned char)itemp;
        }
        buf0[i] = '\0';//printf("%d",buf0[1]);
    }



	//fread(buf,sizeof(char),16,file_input);
	memcpy(buf,buf0+t*16,16);
	//for(int y=0;y<16;y++){
	//printf("%c",buf[y]);
	//}
	//printf("\n");
//        for (i = 0; i < 16; i++) {
//            buf[i] ^= iv[i];  // XOR with the previous block's ciphertext or IV
//        }
	//memcpy(iv, buf, 16);
//        gm_sm4_crypt(key, 1, buf, buf1);
	//memcpy(buf2 + t * 16, buf1, 16);
  //      memcpy(iv, buf1, 16);  // 更新 IV
	for (int j = 0; j < 16; j++) {
        	sprintf(res + j * 2, "%02X", (buf[j] & 0xFF));
    	}
    printf("r = %s\n", res);
    fputs(res,file);
    }
    ...
    FILE *file1 = fopen("enoutput.txt","w");
 ...
 //转化成2进制
    if (f != NULL && strlen(f) > 0) {
        ...
        unsigned char temp[16];  // 临时存储解密后的密文块
    memcpy(temp, buf0 + q * 16, 16);
	//fread(temp,sizeof(char),16,file_input);
	
    
    
    memcpy(buf , temp,16);

        gm_sm4_crypt(key, 0, buf, buf1);
        for (i = 0; i < 16; i++) {
            buf1[i] ^= iv[i];  // XOR with the previous block's ciphertext or IV
        }
        memcpy(iv, temp, 16);  // 更新 IV
	for (int j = 0; j < 16; j++) {
        sprintf(res + j * 2, "%02X", (buf1[j] & 0xFF));
    ...
    // 输出结果
    /*for (i = 0; i < 16; i++) {
        sprintf(res + i * 2, "%02X", (buf1[i] & 0xFF));
...
void* sm4_thread1(void* args){
    struct shuju * sm4_shuju = (struct shuju*)args;
    unsigned char buf1[33000] = {0x0};
    int i;
    unsigned char buf[33000] = {0x0};
    strcpy(buf,sm4_shuju->buf);
    int iLen;
    iLen = sm4_shuju->iLen;
    gm_sm4_context* ctx = sm4_shuju->ctx;
    int isCBC = sm4_shuju->isCBC;
    int pkcs7 = sm4_shuju->pkcs7;
    unsigned char key[16] = {
        0x6B, 0x8B, 0x45, 0x67, 0x32, 0x7B, 0x23, 0xC6, 
        0x64, 0x3C, 0x98, 0x69, 0x66, 0x33, 0x48, 0x73
    };
    int mod = sm4_shuju->mod;
    for(i = 0;i < 10000000; i++){
        gm_sm4_init(&ctx, key, mod, pkcs7, isCBC ? key : NULL);
        // 测试内容长度是16的倍数的case
        if(iLen > 32) {
            int tmpLen = iLen - 32;
...
void test_sm4_ecb_cbc(int isCBC, int pkcs7, const char * input_hex, int iHexLen, const char * res_hex, int rHexLen) {
    ...
    struct shuju sm4_shuju1 = {{0x0},NULL,0};
    struct shuju * sm4_point1 = &sm4_shuju1;
    sm4_shuju1.ctx = &ctx;
    strcpy(sm4_shuju1.buf,buf);
    sm4_shuju1.iLen = iLen;
    sm4_shuju1.isCBC = isCBC;
    sm4_shuju1.pkcs7 = pkcs7;
	sm4_shuju1.mod = 1;

    struct shuju sm4_shuju2 = {{0x0},NULL,0};
    struct shuju * sm4_point2 = &sm4_shuju2;
    sm4_shuju2.ctx = &ctx;
    strcpy(sm4_shuju2.buf,buf);
    sm4_shuju2.iLen = iLen;
    sm4_shuju2.isCBC = isCBC;
    sm4_shuju2.pkcs7 = pkcs7;
sm4_shuju2.mod = 1;

    struct shuju sm4_shuju3 = {{0x0},NULL,0};
    struct shuju * sm4_point3 = &sm4_shuju3;
    sm4_shuju3.ctx = &ctx;
    strcpy(sm4_shuju3.buf,buf);
    sm4_shuju3.iLen = iLen;
    sm4_shuju3.isCBC = isCBC;
    sm4_shuju3.pkcs7 = pkcs7;
sm4_shuju3.mod = 1;

    struct shuju sm4_shuju4 = {{0x0},NULL,0};
    struct shuju * sm4_point4 = &sm4_shuju4;
    sm4_shuju4.ctx = &ctx;
    sm4_shuju4.isCBC = isCBC;
    sm4_shuju4.pkcs7 = pkcs7;
    strcpy(sm4_shuju4.buf,buf);
    sm4_shuju4.iLen = iLen;
sm4_shuju4.mod = 1;

    struct shuju sm4_shuju5 = {{0x0},NULL,0};
    struct shuju * sm4_point5 = &sm4_shuju5;
    sm4_shuju5.ctx = &ctx;
    sm4_shuju5.isCBC = isCBC;
    sm4_shuju5.pkcs7 = pkcs7;
    strcpy(sm4_shuju5.buf,buf);
    sm4_shuju5.iLen = iLen;
sm4_shuju5.mod = 1;

    struct shuju sm4_shuju6 = {{0x0},NULL,0};
    struct shuju * sm4_point6 = &sm4_shuju6;
    sm4_shuju6.ctx = &ctx;
    sm4_shuju6.isCBC = isCBC;
    sm4_shuju6.pkcs7 = pkcs7;
    strcpy(sm4_shuju6.buf,buf);
    sm4_shuju6.iLen = iLen;
sm4_shuju6.mod = 1;

    struct shuju sm4_shuju7 = {{0x0},NULL,0};
    struct shuju * sm4_point7 = &sm4_shuju7;
    sm4_shuju7.ctx = &ctx;
    sm4_shuju7.isCBC = isCBC;
    sm4_shuju7.pkcs7 = pkcs7;
    strcpy(sm4_shuju7.buf,buf);
    sm4_shuju7.iLen = iLen;
sm4_shuju7.mod = 1;

    struct shuju sm4_shuju8 = {{0x0},NULL,0};
    struct shuju * sm4_point8 = &sm4_shuju8;
    sm4_shuju8.ctx = &ctx;
    sm4_shuju8.isCBC = isCBC;
    sm4_shuju8.pkcs7 = pkcs7;
    strcpy(sm4_shuju8.buf,buf);
    sm4_shuju8.iLen = iLen;
sm4_shuju8.mod = 1;

    pthread_create(&th1,NULL,sm4_thread1,sm4_point1);
    pthread_create(&th2,NULL,sm4_thread2,sm4_point2);
    pthread_create(&th3,NULL,sm4_thread3,sm4_point3);
    pthread_create(&th4,NULL,sm4_thread4,sm4_point4);
    pthread_create(&th5,NULL,sm4_thread5,sm4_point5);
    pthread_create(&th6,NULL,sm4_thread6,sm4_point6);
    pthread_create(&th7,NULL,sm4_thread7,sm4_point7);
    pthread_create(&th8,NULL,sm4_thread8,sm4_point8);
    pthread_join(th1,NULL);
    pthread_join(th2,NULL);
    pthread_join(th3,NULL);
    pthread_join(th4,NULL);
    ...
    unsigned int message_length = 16 * (80); // Total message length for 100,000 iterations
    unsigned int throughput = (unsigned int) ((message_length * 8) / (cpu_time_used / 8)); // Mbps calculation
    printf("Throughput: %u Mbps\n", throughput);
...
/*        for(i = 0; i < 10000000; i++) {
    ...
    cpu_time_used = ((double) (end - start)) / CLOCKS_PER_SEC;
    printf("cputime = %f\n",cpu_time_used);
    unsigned int message_length1 = 16 * (80); //Total message length for 100,000 iterations
    unsigned int throughput1 = (unsigned int) ((message_length1 * 8) / (cpu_time_used/8)); // Mbps calculation
    printf("Throughput: %u Mbps\n", throughput1);
    return;
}
...
void test(const char ** argv) {
    ...
    if (strcmp(argv[1], "test_gm_sv_once") == 0) {
        char* key_hex = argv[2];
        char* pubKey_hex = argv[3];
        char* sig_hex = argv[4];
        char* input = argv[5];
        unsigned int iLen = strlen(input);
        int algType = atoi(argv[6]);
        test_gm_sv_once(key_hex, pubKey_hex, sig_hex, input, algType);
    }
    ...
    if(strcmp(argv[1], "gm_sm3") == 0) {
        test_gm_sm3("DC7E07FF06247D00B4A8D1837C8F8B2A26C3C67C2EEE81B1E7CF9400B51891CB", 32, "DC7E07FF06247D00B4A8D1837C8F8B2A26C3C67C2EEE81B1E7CF9400B51891CB");
    }

	 if(strcmp(argv[1], "gm_sm3_once") == 0) {
    	char* input = argv[2];
        int iLen = atoi(argv[3]);
        test_gm_sm3_once(input, iLen);
    }
    ...
    if(strcmp(argv[1], "sm2_verify") == 0) {
      ...
      if(strcmp(argv[1], "gm_sm4_once") == 0) {
	unsigned char key[16];
	 // memcpy(key, argv[2], 16);
        //unsigned char* key = argv[2];
        //unsigned char* input = argv[4];
	unsigned char input[16];
	//memcpy(input, argv[4], 16);
        int algType = atoi(argv[18]);
        for(int t=0;t<16;t++){
	   key[t]=(unsigned char)strtol(argv[2+t],NULL,0);	    	
	}
	for(int t=0;t<16;t++){
	   input[t]=(unsigned char)strtol(argv[19+t],NULL,0);
	}
	...
	//printf("%X",input[0]);
        test_gm_sm4(key,algType , input);//1加密，0解密
    }

   if(strcmp(argv[1], "gm_sm4_cbc_once") == 0) {
        unsigned char key[16];
	 // memcpy(key, argv[2], 16);
        //unsigned char* key = argv[2];
        unsigned char* input = argv[19];
	//unsigned char input[16];
	//memcpy(input, argv[4], 16);
        int algType = atoi(argv[18]);
        for(int t=0;t<16;t++){
	...
	//printf("%X",input[0]);
        test_gm_sm4_cbc(key,algType , input);//1加密，0解密
    }
